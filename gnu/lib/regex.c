...
/* Using the compiled pattern in BUFP->buffer, first tries to match the
   ...
   only at STARTPOS; in general, the last start tried is STARTPOS + RANGE.

   If RANGE is negative, search backwards.  The starting positions
   tried are STARTPOS, STARTPOS - 1, etc., through STARTPOS + RANGE.
     ...
     int stop;
{
  ...
  while (1)
      ...
      if (fastmap && startpos < total_size && !bufp->can_be_null)
	{
      ...
      /* If we can't match null string, and that's all we have left, fail.  */
