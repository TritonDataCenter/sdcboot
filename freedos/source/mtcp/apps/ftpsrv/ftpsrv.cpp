...
static char Msg_550_Bad_Drive_Letter[] =     "550 Invalid or inactive drive letter in path: %s" _NL_;
...
int isDriveInValidTable( int driveLetter ) {
...
void doMkd( FtpClient *client, char *nextTokenPtr ) {
  ...
  if ( rc != 0 ) {
    ...
    client->addToOutput_var( "550 %s: already exists" _NL_, (tmpPath+ftpRootLen) );
  }
...
void doCwd( FtpClient *client, char *nextTokenPtr ) {
  ...
  if ( !isSandbox ) {
    ...
    if ( !isDrivePrefixPresent( fullpath ) || !isDriveInValidTable( fullpath[7] ) ) {
      client->addToOutput_var( Msg_550_Bad_Drive_Letter, parm );
      return;
    }
...
int formFullPath_DataXfer( FtpClient *client, char *outBuffer, int outBufferLen, char *filespec ) {
  ...
  switch ( rc ) {
    ...
    case 0: { break; }
    case 1: { endDataTransfers( client, "550 Bad path" _NL_ ); break; }
...
void doDataXfer( FtpClient *client, char *parms ) {
  ...
  if ( client->dataXferState == FtpClient::DL_Closing ) {
    ...
    Utils::getNextToken( parms, client->filespec, USR_MAX_PATHFILE_LENGTH );
...
static int readConfigParms( void ) {
  ...
  if ( Utils::getAppValue( "FTPSRV_SESSION_TIMEOUT", tmpBuffer, 10 ) == 0 ) {
    ...
    if ( (tmpVal > 59) && tmpVal < 1201 ) {
      ...
      addToScreen( 1, "FTPSRV_SESSION_TIMEOUT must be between 60 and 1200 seconds\n" );
      return 1;
    }
  }
...
static int initSrv( void ) {
  ...
  // Populate list of active drive letters.  We have to do this before
  // reading the config parms because we need to know the list of active
  // drive letters first.
  scanValidDrives( );
  ...
  addToScreen( 1, "Real IP address: %d.%d.%d.%d, Pasv response IP addr: %d.%d.%d.%d\n",
...
static unsigned _my_dos_findfirst( const char *path, unsigned attributes, struct find_t *buffer ) {
  ...
  if ( path[0] == '/' ) {
    ...
    // A Unix stile path was passed in, so they are at the root of the
...
void scrollMsgArea( int lines ) {
  memmove( (void *)(Screen_base + 2*160), (void *)(Screen_base + (2+lines)*160), ((ScreenRows-2)-lines)*160 );
  uint16_t far *start = (uint16_t far *)(Screen_base + (ScreenRows-lines)*160);
  fillUsingWord( start, (7<<8), lines*80*2);
}
...
void addToScreen( int writeLog, const char *fmt, ... ) {
  ...
  if ( bytesOut > 511 ) {
    ...
    strcpy( logLineBuffer, "<INTERNAL LOGGING ERROR>" );
    bytesOut = 24;
  }
  ...
  uint16_t far *start = (uint16_t far *)(Screen_base + ((ScreenRows-linesToScroll)*80*2));
...
void redrawStatusLine( void ) {
  ...
  myCprintf( 13, 0, 0x0F, "Total Connections:" );
  myCprintf( 32, 0, 0x07, "%5d", SessionCounter );
  myCprintf( 39, 0, 0x0F, "Active Sessions:" );
  myCprintf( 56, 0, 0x07, "%2d", FtpClient::activeClients );
...
void initScreen( void ) {
  ...
  // Draw separator line
  fillUsingWord( (uint16_t far *)(Screen_base + 160), (7<<8|196), 80 );
