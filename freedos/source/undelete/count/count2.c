...
#ifdef DEBUG
  printf("  [COUNT2: %Fp->", wordp);
#endif
  wordp = (word far *)MK_FP(wordp[0], 0x16); /* wordp[0] was pPSP */
#ifdef DEBUG
  printf("%Fp-> parent parent PSP at %4.4x:0, ENV at ",
	 wordp, wordp[0]); /* wordp[0] is ppPSP */
#endif
  wordp = (word far *)MK_FP(wordp[0], 0x2c);
  envp = (const char far *)MK_FP(wordp[0], 0);
#ifdef DEBUG
  printf("%4.4x:0]\n\n", wordp[0]); /* anyPSP[2c] is ENV of anyPSP */
#endif
