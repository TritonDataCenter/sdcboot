...
/* Connected to DOS drivers: Imre Leber and Eric Auer 2002 - 2004 */
...
/* dosfstools 2.11 use only GLIBC lseek, no longer use 5 arg kernel llseek */
/* GLIBC: llseek int fd, loff_t offset, int whence */
/* _llseek int fd, uint32 offset_hi, uint32 offset_lo, loff_t * actual, */
/*   int whence --> returns zero if okay and actual offset in actual    */
#define llseek lseek
...
void fs_read(loff_t pos,int size,void *data)
{
	...
	pdie("Seek to %lld failed",pos);
    if ((got = read(fd,data,size)) < 0)
    	pdie("Read %d bytes at %lld failed",size,pos);
    if (got != size)
...
void fs_write(loff_t pos,int size,void *data)
{
	...
	if ((did = write(fd,data,size)) == size)
	    return;
	if (did < 0)
	    pdie("Write %d bytes at %lld failed",size,pos);
...
int fs_close(int write)
{
    ...
    if (write) fs_flush();
	    ...
	    next = changes->next;
	    myfree(changes->data, changes->size);
	    myfree(changes, sizeof(CHANGE));
	    changes = next;
	}

    if (close(fd) < 0)
	pdie("Error closing file system");
