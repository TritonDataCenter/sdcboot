...
#include "version.h"
#include <crt0.h>
int _crt0_startup_flags = _CRT0_FLAG_LOCK_MEMORY | _CRT0_FLAG_DROP_EXE_SUFFIX;
    /* disable all swapping / VM, and suppress the ".exe" in argv[0] string */
    /* also nice: _CRT0_FLAG_FILL_SBRK_MEMORY _CRT0_FLAG_FILL_DEADBEEF */
#else
#include "../version.h"
#endif
...
#include <unistd.h>
#ifndef __DJGPP__
#include <getopt.h>	/* in DJGPP there is some getopt in unistd */
#endif
...
int main(int argc,char **argv)
{
	...
	switch (c) {
		...
		break;
	    case 'v': /* VERSION number is printed below anyway, v is VERBOSE */
		verbose = 1;
    ...
    if (optind != argc-1) usage(basename(argv[0]));
