...
static void check_backup_boot(DOS_FS *fs, struct boot_sector *b, int lss)
{
	...
	if (!interactive || get_key("12","?") == '1') {
	    ...
	    fs_write((loff_t)offsetof(struct boot_sector,backup_boot), /* loff_t was correct! */
...
static void init_fsinfo(struct info_sector *i)
{
    ...
    i->boot_sign = CT_LE_L(0xaa550000); /* dosfstools 2.11: test only 16 bits? */
}
...
static void read_fsinfo(DOS_FS *fs, struct boot_sector *b,int lss)
{
	...
	if (interactive && get_key("12","?") == '1') {
		...
		init_fsinfo(&i);
		fs_write((loff_t)s*lss,sizeof(i), &i); /* loff_t was correct! */
		b->info_sector = CT_LE_W(s);
		fs_write((loff_t)offsetof(struct boot_sector,info_sector), /* loff_t was correct! */
	...
	if (i.boot_sign != CT_LE_L(0xaa550000)) /* dosfstools 2.11: test only 16 bits? */
	    printf( "  Offset %llu: 0x%08x != expected 0x%08x\n",
		    (unsigned long long)offsetof(struct info_sector,boot_sign),
		    CF_LE_L(i.boot_sign),0xaa550000); /* dosfstools 2.11: test only 16 bits? */
	if (interactive)
...
void read_boot(DOS_FS *fs)
{
    ...
    unsigned fat_length;
    loff_t data_size; /* loff_t was correct! */
    ...
    fs_test((loff_t)((total_sectors & ~1)-1)*(off_t)logical_sector_size, /* loff_t was correct! */
	    logical_sector_size);
    fat_length = CF_LE_W(b.fat_length) ?
		 CF_LE_W(b.fat_length) : CF_LE_L(b.fat32_length);
    fs->fat_start = (loff_t)CF_LE_W(b.reserved)*logical_sector_size; /* loff_t was correct! */
    fs->root_start = ((loff_t)CF_LE_W(b.reserved)+b.fats*fat_length)* /* loff_t was correct! */
      logical_sector_size;
    fs->root_entries = GET_UNALIGNED_W(b.dir_entries);
    fs->data_start = fs->root_start+ROUND_TO_MULTIPLE(fs->root_entries <<
      MSDOS_DIR_BITS,logical_sector_size);
    data_size = (loff_t)total_sectors*logical_sector_size-fs->data_start; /* loff_t was correct! */
