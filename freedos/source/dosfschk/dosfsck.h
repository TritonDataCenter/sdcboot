...
#include <sys/types.h>
#include "common.h"	/* include local msdos_fs.h and local types if DOS */
...
struct info_sector {
    ...
    __u32	reserved2[3];
    __u32	boot_sign;	/* the whole "00 00 55 aa" matters in 2.10 */
}; /* Linux dosfsck 2.11 uses 16 bit boot_sign, DOS dosfsck 2.11 uses 32 bit */

typedef struct {
    __u8	name[8],ext[3];	/* name and extension, __u8 since 2.11 */
...
typedef struct {
    unsigned long value;
    /* unsigned long reserved; -> 2.11b: mask reserved bits out, saves RAM but takes CPU */
    DOS_FILE *owner;
