...
static BOOL RealyReadVirtualFile(VirtualDirectoryEntry* entry, unsigned long pos, char* data, unsigned long length)
{
    ...
    if (pos + length > entry->length) return FALSE;
...
{ 
    ...
    for (i=0; i < MAX_ENTRIES; i++)
    {
        if ((VirtualDirectory[i].used) &&
            (VirtualDirectory[i].lowestClusterInvolved < last))
        {
            last = VirtualDirectory[i].lowestClusterInvolved;
        }
    }
...
static CLUSTER PrepareVirtualFat(RDWRHandle handle, unsigned long n)
{
    /* Prepare a fat to store n bytes */


}

static BOOL SetNthVirtualCluster(RDWRHandle handle, CLUSTER fat, unsigned long n, CLUSTER value)
{
    unsigned long labelspercluster, sectorspercluster;

    SECTOR sector;
    unsigned offset;
    CLUSTER fat = GetVirtualFatCluster(handle, fat, n);
    if (!fat) return FALSE;

    sectorspercluster = GetSectorsPerCluster(handle);
    if (!sectorspercluster) return FALSE;

    labelspercluster = ((sectorspercluster * BYTESPERSECTOR) / sizeof(CLUSTER)) - 1;  
    
    sector = ConvertToDataSector(handle, fat);
    if (!sector) return FALSE;

    if (n < (BYTESPERSECTOR / sizeof(CLUSTER)) - 1)
    {
        offset = n + 1;    
    }
    else
    {
       n -= ((BYTESPERSECTOR / sizeof(CLUSTER)) - 1);

       sector += (n % labelspercluster) / (BYTESPERSECTOR / sizeof(CLUSTER)) + 1;
       offset  = (n % labelspercluster) % (BYTESPERSECTOR / sizeof(CLUSTER));
    }

    if (!ReadDataSectors(handle, 1, sector, SectorBuffer))
        return FALSE;

    ((CLUSTER*)SectorBuffer)[offset] = value;
}

static BOOL GetNthVirtualCluster(RDWRHandle handle, CLUSTER fat, unsigned long n)
{
    unsigned long labelspercluster, sectorspercluster;
    ...
    return ((CLUSTER*)SectorBuffer)[offset];
}

static CLUSTER GetVirtualFatCluster(RDWRHandle handle, CLUSTER fat, unsigned long n)
{
    SECTOR sector;
