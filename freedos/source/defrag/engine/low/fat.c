...
unsigned long GetBytesInFat(RDWRHandle handle)
{
    ...
    unsigned long fatlabelsize, labelsinfat;

    fatlabelsize = (unsigned long) GetFatLabelSize(handle);
    labelsinfat  = (unsigned long) GetLabelsInFat(handle);
    
    retVal = (fatlabelsize * labelsinfat) / 8L;

    if (fatlabelsize == FAT12)
    {
        if (labelsinfat % 2 == 1)
            retVal++;
    }
